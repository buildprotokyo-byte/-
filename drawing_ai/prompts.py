...
room要素については、図面に面積(m2または畳数)が印字されていれば必ずarea_textに
書き写してください。壁ポリゴンからの面積算出ができない図面(スキャン図面等)でも、
印字された面積があればそれを使って数量化できるようにするためです。

面積の印字が無い場合は、代わりにその部屋の横方向・縦方向それぞれの境界に記載されて
いる寸法数字を width_text / depth_text に書き写してください。CADの整列した寸法線
だけでなく、現地実測で壁の近くに個別に書き込まれた数字(一直線に並んでいない場合)
でも構いません。画像を見て、その数字がどの壁・どの部屋の境界を指しているかを
あなた自身で判断してください。どの数字かを特定できない場合は、無理に埋めず空文字に
してください。
出力は必ず有効なJSONのみ。"""
...
{{
  ...
  "elements": [
    {{
      ...
      "label_ja": "部材・部屋の名称",
      "attributes": {{"補足キー": "補足値"}},
      "area_text": "room要素で、面積が図面に印字されている場合のみ原文のまま記載(例: 5.3m2, 12.5畳, 5.3m2|3.2畳)。印字が無ければ空文字。読み取れない数値を計算で作り出さないこと",
      "width_text": "room要素で、面積の印字が無い場合のみ: その部屋の横方向の境界に実際に記載されている寸法数字を、画像を見てそのまま書き写す(例: 3600)。CADの整列した寸法線でなくても、現地実測で壁の近くに手書き/記載された数字でも構わない。どの数字か特定できなければ空文字(創作しない)",
      "depth_text": "同様に、その部屋の縦方向の境界の寸法数字。特定できなければ空文字",
      "confidence": 0.0から1.0
    }}
  ]
}}
...
このページが質疑書(RFI)形式(No./工種/質疑事項/回答の表)である場合は、roomsではなく
qa_itemsに1行ずつ書き写してください。回答が「質疑No.6参照」のように他の行を指している
場合は、参照先の番号が分かるようそのまま回答欄に書き写してください(参照を自分で
解決しようとしないでください。後工程が機械的に解決します)。
出力は必ず有効なJSONのみ。"""
...
{{
  ...
  "qa_items": [
    {{
      "item_no": "質疑書のNo.(整数)。読み取れなければnull",
      "category": "工種(建築/設備/電気/共通 等)",
      "question": "質疑事項の原文",
      "answer": "回答の原文(「質疑No.6参照」のような参照表現もそのまま書き写す)"
    }}
  ]
}}
