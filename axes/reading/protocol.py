...
採否について正直に書いておくこと
------------------------------------
**この形(方式A2)は、2026-09-21 の反復3回の測定で難易度4だけ方式Aを
下回った(A2=50/54 対 A=54/54)が、判定基準を先に宣言したうえで反復10回に
増やして測り直した結果(2026-09-22)、この劣化は再現しなかった。** 難易度4は
方式A・A2とも180/180で差ゼロ(Fisherの正確検定 p=1.0)。判定基準は
`docs/stage05_a_vs_a2_remeasurement_criteria_2026-09-22.md`(commit 816e95b)、
結果は `docs/stage05_a_vs_a2_10reps_report.md`(commit aa9e7bc)。

**結論: A2は「害が無いことは示せたが、利点はまだ示せていない」という
位置づけで確定した(おーちゃん判断、2026-09-22)。既定は方式Aのまま、
A2のコードは残し `stage_half=True` を明示指定したときだけ使う。**
方式Aが難易度1・3・4いずれでも天井(99.4〜100%)に張り付いているため、
**「段階0.5が精度を上げるか」は今も測れていない。** 測るには方式Aでも
誤答が出るほど難しい設問・条件が要るが、**この設計・測定は今は着手しない**
(2026-09-22 おーちゃん判断)。示せているのは「方式Bのように工程を分けると
下がる現象が実在する」ことと、「難易度4の限られた罠1種類では、反復10回で
A2がAを下回る有意な証拠は無い」ところまでである。
"""
...
class ReadingRequest:
    ...
    #:
    #: 2026-09-22 におーちゃんの判断で既定から外した。難易度4の反復3回の
    #: 測定で A2 = 50/54 に対し A = 54/54 となり、採用根拠だった「害が無い」
    #: が崩れたためである(v8 10章23項②・24項)。その後、判定基準を先に
    #: 宣言したうえで反復10回に増やして測り直したところ、この劣化は
    #: 再現しなかった(難易度4はA・A2とも180/180、Fisher検定 p=1.0。
    #: `docs/stage05_a_vs_a2_10reps_report.md`、commit aa9e7bc)。
    #: **それでも既定は方式Aのまま確定している** — A2は「害が無いことは
    #: 示せたが、利点はまだ示せていない」位置づけであり(v8 13-3節)、
    #: **コードは残し、明示的に指定したときだけ使う。**
    #:
