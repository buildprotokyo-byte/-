...
class Test図面の種類:
    ...
    def test_案件から作り直して足した種類(self):
        """**先に並べた一覧に当たらなかった 7 件を見て足した 2 つ。**
        一覧に図面を合わせにいったのではなく、図面から一覧を作り直した。"""
        assert classify("既存撤去図") == "撤去図"
        assert classify("電気位置図") == "設備図"

    def test_物の名前が付いた図面は種類が決まらない(self):
        """**図面の名前が、図面の種類ではなく描かれている物を指すことがある。**
        そのときは名前だけでは何の図面か決まらないので、その他に落とす。"""
        assert classify("収納") == "その他"
