...
PageKind = Literal["平面図", "展開図", "建具表", "仕上表", "設備図", "凡例", "その他"]
...
#: 繰り返す記号が**出てくると見込まれる**ページの種類。電気の記号は
#: 平面図にも設備図にも載る。
#:
#: **これは探索の範囲ではない**(`DOOR_ARC_EXPECTED_PAGE_KINDS` と同じ扱い)。
#: 探索は宣言があってもなくても全ページで行い、この並びは読めた群が人の宣言と
#: 食い違っているかを判定するためだけに使う。
#:
#: 2026-09-23 まではこの並び(当時の名前は ``REPEATED_SYMBOL_PAGE_KINDS``)が
#: **探索そのものを止めていた。** PR #37 が開き戸で直した形が、#37 を含まない
#: ブランチで書かれた A-1 に残っていた(K-04 の 5)。
REPEATED_SYMBOL_EXPECTED_PAGE_KINDS: tuple[PageKind, ...] = ("平面図", "設備図")

#: 室の輪郭が**出てくると見込まれる**ページの種類。展開図・建具表には
#: 室の輪郭が無いはずである。
#:
#: **これも探索の範囲ではない。** 使い方は `REPEATED_SYMBOL_EXPECTED_PAGE_KINDS`
#: と同じ。2026-09-23 までは(当時の名前は ``ROOM_OUTLINE_PAGE_KINDS``)
#: 探索そのものを止めていた(A-2)。
#:
#: 設備図は平面図を下敷きに描かれることが多く、室の輪郭が出うるが、
#: **ここには入れていない**(値は直す前のまま)。入れるかどうかは
#: おーちゃんの判断待ち(`docs/page_kind_gate_report.md`)。
ROOM_OUTLINE_EXPECTED_PAGE_KINDS: tuple[PageKind, ...] = ("平面図",)

#: 「名前 ↔ 記号」の対応を読むページの種類。
#: **人が「凡例」と宣言したページからしか名前は取らない。** 平面図の上で
#: 室名が記号の左に並んでいるだけの並びを凡例と読み違えると、名前が捏造される。
LEGEND_PAGE_KINDS: tuple[PageKind, ...] = ("凡例",)
