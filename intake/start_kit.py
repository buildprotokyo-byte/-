...
#: 繰り返す記号を探すページの種類。電気の記号は平面図にも設備図にも載る。
#: 種類が宣言されていないページは今までどおり全部探す(前提を必須にしないため)。
REPEATED_SYMBOL_PAGE_KINDS: tuple[PageKind, ...] = ("平面図", "設備図")

#: 室の輪郭を探すページの種類。展開図・建具表には室の輪郭が無い。
ROOM_OUTLINE_PAGE_KINDS: tuple[PageKind, ...] = ("平面図",)
...
#: 「判断できない」と人が正直に答えた種類。**宣言しなかったのと同じに扱い、
#: 読み取りの関門にしない**(K-36、2026-09-25)。
#:
#: 周36 の実測で、34 ページを「その他」と宣言すると入口の数量が
#: 2,887 → 166 件(94% 減)になった。宣言しなければ関門は無いので、
#: **正直に答えるほど損をする作り**だった。
UNDECIDED_PAGE_KIND: PageKind = "その他"
